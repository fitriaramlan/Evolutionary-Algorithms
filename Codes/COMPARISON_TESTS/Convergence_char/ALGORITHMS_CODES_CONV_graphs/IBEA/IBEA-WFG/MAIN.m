...
%-----------------------------------------------------------------------------------------      
    ...
    for Gene = 1 : Generations
        MatingPool                 = TournamentSelection(2,N,-CalFitness(F,kappa));
        Offspring                  = F_operator(Population(MatingPool',:),Boundary);
         NewFunctionValue            = wfg(Offspring, M, k, l, testNo);
        [Population,F] = EnvironmentalSelection([Population;Offspring],[F;NewFunctionValue],N,kappa);    
        
       Gene         
    end

    F_output(Population,toc,'IBEA-WFG',testNo,M,k,l,run);
