...
%-----------------------------------------------------------------------------------------      
    ...
    for Gene = 1 : Generations
        MatingPool   = randi(N,1,2*N);
        Offspring    = F_operator(Population(MatingPool',:),Boundary);
        NewFunctionValue           = F_DTLZ(Offspring,Problem,M,K);
        [Population,FunctionValue] = EnvironmentalSelection([Population;Offspring],[FunctionValue;NewFunctionValue],N,rand*0.2+0.4);     
        
   %%convergence graphs AND igd,gd,DELTA CALCUL;ATIONS
                    if M==4
                        load D:\trinadh\Convergence_char\RESULT-LOOP-DTLZ\D1_M4.mat;
                        True_PF = KK;
                        
                    elseif M==6
                        load D:\trinadh\Convergence_char\RESULT-LOOP-DTLZ\D1_M6.Mat;
                        True_PF = KK;
                    elseif M==8
                        load D:\trinadh\Convergence_char\RESULT-LOOP-DTLZ\D1_M8.Mat;
                        True_PF = KK;
                    else
                        load D:\trinadh\Convergence_char\RESULT-LOOP-DTLZ\D1_M10.Mat;
                        True_PF = KK;
                    end
                    ...
                    save(['D:\trinadh\Convergence_char\ALGORITHMS_CODES_CONV_graphs\SRA\SRA-DTLZ\IGD_GD_DELTA_gen\IGD_GD_DELTA_gen_D' num2str(Problem) '_M' num2str(M) '_run' num2str(run) '.mat'],'result');
            ...
            for r=1:Runs
                eval(['load D:\trinadh\Convergence_char\ALGORITHMS_CODES_CONV_graphs\SRA\SRA-DTLZ\IGD_GD_DELTA_gen\IGD_GD_DELTA_gen_D' num2str(Problem) '_M' num2str(M) '_run' num2str(r) '.mat']);
                tot=tot+result;
                
            end
            FinalMean=tot./Runs;
            eval(['save D:\trinadh\Convergence_char\FINAL_PLOTvalues\SRA','_', num2str(Problem),'_',num2str(M),'.mat']);
