function MatingPool = MatingSelection_GR_GCD_SOB(PopObj,div)
...
% Copyright (c) 2016-2017 BIMK Group
    ...
    GCD = sum(GD,2);
     %% sum of OBJ
       fmaxi   = repmat(max(PopObj,[],1),N,1);
    fmini   = repmat(min(PopObj,[],1),N,1);
    PopObj = (PopObj-fmini)./(fmaxi-fmini);
    fpr    = mean(PopObj,2);
    ...
    for i = 1:N
        ...
        elseif GR(Parent1(i)) > GR(Parent2(i))
            ...
            if rand < 0.5 %%% giving equal probability to both GCD and SOB
                ...
                if fpr(Parent1(i)) < fpr(Parent2(i))
                    MatingPool(i) = Parent1(i);
                else
                    MatingPool(i) = Parent2(i);
                end
