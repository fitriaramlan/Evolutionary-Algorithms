...
%-----------------------------------------------------------------------------------------      
  for run = 1 : Runs
    ...
    FrontValue                    = ENS(F,N,'all');           % non-dominated sort, and return the front number of each solution
...
%-----------------------------------------------------------------------------------------      
    ...
    for Gene = 1 : Generations
        ...
        % generate offspring
        NewPopulation    = F_generator(Population,F,MaxValue,MinValue,FrontValue,DistanceValue);
        Population       = [Population;NewPopulation];              % combine the two populations
        F                = wfg(Population, M, k, l, testNo);            % calculate the objective function values
       
        [FrontValue,NoF] = ENS(F,N,'half');          	% non-dominated sort, and return the front number of each solution
     ...
     F_output(Population,toc,'NSGAII-SDE_W',testNo,M,k,l,run);
