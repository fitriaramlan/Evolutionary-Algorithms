function [FrontValue,NoF] = A_SORNS9 (Population,operation)
...
%               NoF: number of total fronts
    ...
    Population = (Population - repmat(min(Population,[],1),N,1))./repmat((max(Population,[],1) - min(Population,[],1)),N,1);
    
    [~,rank] = sort(sum(Population,2));
    ...
    for i = 2 : N
        ...
        while 1
            ...
            for j = 1:size(F{k},2) 
                ...
                [~,aj] = max(Population(F{k}(j),:));
                at = [ai aj];
                x = 2;
              
                for j2 = 1 : M
                    % if Population(rank(i),at(j2)) < Population(F{k}(j),at(j2))
