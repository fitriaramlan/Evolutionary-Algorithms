function [FrontValue,NoF] = ENS_BS (Population,operation)
% Efficient non-dominated sort on binary search strategy
...
%               NoF: number of total fronts
    ...
    for i = 2 : N
        kmin = 0;                           % the lower bound for checking
        kmax = NoF;                         % the upper bound for checking
        k = fix((kmax + kmin) / 2 + 0.5);   % the front now checked
        while 1
